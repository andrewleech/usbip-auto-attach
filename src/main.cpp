...
std::optional<std::string> find_usbip(const std::string& user_path = "") {
    ...
    // This part is omitted as getting the executable path reliably without platform specifics is complex.
...
std::optional<Args> parse_args(int argc, char* argv[]) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (arg == "-v" || arg == "--verbose") {
            ...
            return std::nullopt; // Indicate help requested
...
void print_usage(const char* prog_name) {
    std::cerr << "Usage: " << prog_name << " <host_ip> <busid> [--usbip-path <path>] [-v|--verbose]" << std::endl;
